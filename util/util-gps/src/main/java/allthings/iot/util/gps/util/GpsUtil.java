package allthings.iot.util.gps.util;

import com.google.common.base.Strings;
import allthings.iot.util.gps.enums.CoorType;
import allthings.iot.util.gps.enums.GpsCoorType;
...
public class GpsUtil {
     ...
     * @return
     */
    public static double transformDegree2Decimal(String rationalString) {
        if (Strings.isNullOrEmpty(rationalString)) {
            return 0;
        }

        String degreeStr = rationalString.substring(0, 3);
        char first = degreeStr.charAt(0);

        //判断符号位
        int iFirst = first & 0x07;
        double degrees = Double.parseDouble(iFirst + degreeStr.substring(1));
        String minutesInt = rationalString.substring(3, 5);
        String minutesDec = rationalString.substring(5);
        double minutes = Double.parseDouble(minutesInt.concat(".").concat(minutesDec));
